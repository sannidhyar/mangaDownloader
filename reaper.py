...
import srcs.crawler_reaper as crawler
...
allLinks = []
html = scraper.get('https://reaperscans.com/comics/4384-legendary-blacksmiths-vengeance').text.split('\n')
for line in html:
    if line.startswith('<a href') and 'chapter' in line:
        # print(line.split('"')[1])
        allLinks.append(line.split('"')[1])
for link in allLinks:
    # crawler.starterFunction(link, './test')
    html = scraper.get(link).text.split('/n')
    exit()
